...
### 🎒 中考词库(框架已就绪,待填词)
`decks/zk1.json` 已建好格式示例(20词)。**扩充方法**:按同样格式创建 `zk2.json`、`zk3.json`…放进 `app/src/main/assets/decks/`,并在 `manifest.json` 的 `zk` 词库 `files` 数组里登记即可。词条格式(第5字段=例句的整句中文翻译,供例句填空显示,强烈建议填写):
```json
